...
async def wiki_search(q: str) -> Optional[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            r = await client.get("https://ru.wikipedia.org/w/api.php", params={"action":"opensearch","search":clean,"limit":1,"namespace":0,"format":"json"})
            data = r.json()
            title = data[1][0] if data and len(data)>1 and data[1] else None
            if not title: return None
            s = await client.get(f"https://ru.wikipedia.org/api/rest_v1/page/summary/{title}")
            js = s.json(); extract = js.get("extract") or ""; url = (js.get("content_urls") or {}).get("desktop", {}).get("page", "")
...
async def arxiv_search(q: str) -> Optional[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            r = await client.get("https://export.arxiv.org/api/query", params={"search_query":f"all:{q[:80]}","start":0,"max_results":1})
            xml=r.text; titles=re.findall(r"<title>([^<]+)</title>",xml); paper=titles[1] if len(titles)>1 else None; ids=re.findall(r"<id>(https://arxiv.org/abs/[^<]+)</id>",xml)
