"""AKSI Core control plane.

Provides one stable control surface around the Infinity task runtime:
- task lifecycle/state
- live journal stream (SSE)
- explicit approval tokens for external side effects
- runtime capabilities/health

This module does not expose model chain-of-thought.
"""
...
async def runtime():
    return {
        ...
        "name": "AKSI Core",
        "protocol": "AKSI-VAI/1",
        "architecture": ["identity", "memory", "model", "tools", "policy", "evidence", "decision", "receipt"],
        "task_states": ["CREATED", "PLANNING", "RESEARCHING", "ANALYZING", "VERIFYING", "COMPLETED", "FAILED", "STOPPED", "NEEDS_PERMISSION"],
        "external_actions": "approval_required",
        "chain_of_thought": "not_exposed",
        "timestamp": now(),
    }
    ...
    async def stream():
        ...
        while idle < 30:
            ...
            while sent < len(journal):
                payload = json.dumps(journal[sent], ensure_ascii=False)
                yield f"id: {sent}\nevent: journal\ndata: {payload}\n\n"
                sent += 1
                idle = 0
            if task.get("status") in {"COMPLETED", "FAILED", "STOPPED", "NEEDS_PERMISSION"}:
...
async def grant_approval(task_id: str, approval_id: str):
    ...
    for approval in task.get("approvals", []):
        if approval.get("id") == approval_id:
            ...
            approval.update({"status": "GRANTED", "token": token, "granted_at": now()})
            task["updated_at"] = now()
            persist_task(task)
...
async def revoke_approval(task_id: str, approval_id: str):
    ...
    for approval in task.get("approvals", []):
        if approval.get("id") == approval_id:
            approval.update({"status": "REVOKED", "token": None, "revoked_at": now()})
            task["updated_at"] = now()
            persist_task(task)
...
async def approvals(task_id: str):
    ...
    if not task:
        raise HTTPException(404, "Task not found")
    # Never return live approval tokens from the listing endpoint.
    safe = [{k: v for k, v in a.items() if k != "token"} for a in task.get("approvals", [])]
    return {"ok": True, "approvals": safe}
...
async def consume_approval(task_id: str, body: ApprovalUse):
    ...
    if not task:
        raise HTTPException(404, "Task not found")
    for approval in task.get("approvals", []):
        if approval.get("token") == body.token and approval.get("status") == "GRANTED":
            approval.update({"status": "CONSUMED", "consumed_at": now(), "token": None})
            task["updated_at"] = now()
            persist_task(task)
