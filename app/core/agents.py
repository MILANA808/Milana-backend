...
class ResearchAgent(BaseAgent):
    ...
    async def run(self, task: str, context: Optional[str] = None) -> AgentResult:
        ...
        try:
            async with httpx.AsyncClient(timeout=12.0, headers=WIKI_HEADERS, follow_redirects=True, trust_env=False) as client:
                r = await client.get("https://ru.wikipedia.org/w/api.php", params={"action": "opensearch", "search": q[:80], "limit": 1, "namespace": 0, "format": "json"})
                if r.status_code != 200:
                    return AgentResult(self.name, task, f"Research: Wikipedia HTTP {r.status_code}", False)
                try:
                    data = r.json()
                except Exception:
                    return AgentResult(self.name, task, "Research: некорректный ответ Wikipedia", False)
                title = data[1][0] if data and len(data) > 1 and data[1] else None
                if not title:
                    return AgentResult(self.name, task, f"Research: по «{q[:60]}» статья не найдена.", True)
                ...
                if url:
                    ans += f"\nИсточник: {url}"
                return AgentResult(self.name, task, ans, True, {"source": "wikipedia", "title": title})
...
async def run_swarm(task: str, context: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"ok": not blocked, "task": task, "agents_run": [x["agent"] for x in results], "results": results, "summary": "\n\n".join(summary_parts), "signature": sign_short(task + str(len(results)))[:24]}
